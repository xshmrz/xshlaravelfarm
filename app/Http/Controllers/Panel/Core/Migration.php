<?php
    ...
    class Migration extends Controller {
        ...
        public function store(Request $request) {
            ...
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.migration.edit", $migration->id);
        }
        ...
        public function update(Request $request, $id) {
            ...
            if (!$migration) {
                ...
                return redirect()->route("panel.migration.index");
            }
            $migration->fill($data);
            $migration->save();
            session()->flash('success', trans("app.İşlem Başarılı"));
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.migration.edit", $id);
        }
        public function destroy($id) {
            ...
            return request()->has("redirect") ? redirect()->to(request()->get("redirect"))->withInput() : redirect()->route("panel.migration.index")->withInput();
        }
    }
