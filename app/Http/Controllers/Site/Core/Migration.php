<?php
    ...
    class Migration extends Controller {
        ...
        public function store(Request $request) {
            ...
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("site.migration.edit", $migration->id);
        }
        ...
        public function update(Request $request, $id) {
            ...
            if (!$migration) {
                ...
                return redirect()->route("site.migration.index");
            }
            $migration->fill($data);
            $migration->save();
            session()->flash('success', trans("app.İşlem Başarılı"));
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("site.migration.edit", $id);
        }
        public function destroy($id) {
            ...
            return request()->has("redirect") ? redirect()->to(request()->get("redirect"))->withInput() : redirect()->route("site.migration.index")->withInput();
        }
    }
